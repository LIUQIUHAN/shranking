...
WITH val AS (SELECT (SELECT id
             ...
             FROM (SELECT (SELECT id
                   ...
                   WHERE issue_year = @Issue_year
                     AND parts_cn IN ('中国', '中国香港', '中国澳门', '中国台湾')
                     AND school_code_world IN (SELECT code FROM univ_ranking_dev.univ WHERE outdated = 0)) A)
