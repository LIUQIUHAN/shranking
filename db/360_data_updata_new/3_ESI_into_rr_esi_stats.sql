...
SELECT @Ver_no                                                                                         AS ver_no,
       ( SELECT id FROM univ_ranking_dev.univ B WHERE A.school_code_word = B.code AND B.outdated = 0 ) AS univ_id,
       ...
       IF(school_code_word IN ( SELECT code FROM univ_ranking_dev.univ_cn ), 1, 0)                     AS is_mc_univ,
       0                                                                                               AS is_copy
