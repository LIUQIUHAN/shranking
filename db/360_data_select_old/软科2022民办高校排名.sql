SELECT
	RANK ( ) OVER ( PARTITION BY Z.省市 ORDER BY Z.总得分 DESC ) AS 分省排名,
	RANK ( ) OVER ( PARTITION BY Z.学校类型 ORDER BY Z.总得分 DESC ) AS 分类排名,
	Z.*
FROM
	(
	SELECT
	...
	FROM
		...
		WHERE
			...
			AND type_code = '0014' 
			AND school_code IN ( SELECT school_code FROM rank_tbl_school_info WHERE rank_type = '民办院校' ) 
		GROUP BY
			school_code 
	...
	WHERE
		...
		AND A.type_code = '0014' 
		AND B.rank_type = '民办院校' 
	GROUP BY
		A.school_code 
	) Z 
ORDER BY
