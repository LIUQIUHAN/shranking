...
data_year = [target.get('dataYear', '0') for target in target_info['data']['allTarget']]
sumtype = [target.get('sumtype', '0') for target in target_info['data']['allTarget']]
data_df = pd.DataFrame(data={'id': id_list, 'name': name_list, 'have_detail': have_detail, 'data_year': data_year,
                             'sumtype': sumtype})
data_df.to_excel('C:/Users/Administrator/Desktop/target_data.xlsx', index=False)
data_df = data_df.loc[(data_df['have_detail'] == 1) & (data_df['sumtype'].isin(['qiuhe']))]
...
while count < len(check_list):
    ...
    try:
        for i in range(count, len(check_list)):
            print(i)
            check = check_list[i]
            query_data['indicatorid'] = check[0]
            if len(check[3]) == 4:
                query_data['rangeyear'] = check[3] + '-' + check[3]
            elif len(check[3]) < 4:
                query_data['rangeyear'] = ''
            else:
                query_data['rangeyear'] = check[3]
            ...
            for dic in data_page['data']['indidata']['data']:
                if len(check[3]) == 4:
                    if str(dic[check[3]]) != '0':
                        detail_query_data_year['institution'] = [dic['institutionCode']]
                        detail_query_data_year['indicatorid'] = check[0]
                        detail_query_data_year['rangeyear'] = check[3]
                        detail_data = requests.post(url=detail_query_url, headers=headers, data=json.dumps(detail_query_data_year)).json()
                        if str(dic[check[3]]) != str(detail_data['data']['total']):
                            print(check, dic['institutionName'])
                        # if int(dic[check[3]]) != sum([p['projectWeight'] for p in detail_data['data']['personInfo']]):
                        #     print(check, dic['institutionName'])
                elif len(check[3]) < 4:
                    if str(dic[check[1]]) != '0':
                        detail_query_data['institution'] = [dic['institutionCode']]
                        detail_query_data['indicatorid'] = check[0]
                        detail_data = requests.post(url=detail_query_url, headers=headers, data=json.dumps(detail_query_data)).json()
                        if str(dic[check[1]]) != str(detail_data['data']['total']):
                            print(check, dic['institutionName'])
                else:
                    detail_query_data_year['institution'] = [dic['institutionCode']]
                    detail_query_data_year['indicatorid'] = check[0]
                    for year in range(int(check[3][:4]), int(check[3][5:])+1):
                        if str(dic[str(year)]) != '0':
                            detail_query_data_year['rangeyear'] = str(year)
                            detail_data = requests.post(url=detail_query_url, headers=headers, data=json.dumps(detail_query_data_year)).json()
                            if str(dic[str(year)]) != str(detail_data['data']['total']):
                                print(check, dic['institutionName'], year)
                            # if int(dic[str(year)]) != sum([p['projectWeight'] for p in detail_data['data']['personInfo']]):
                            #     print(check, dic['institutionName'], year)
