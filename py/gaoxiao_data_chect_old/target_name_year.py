...
for i, target_dic in enumerate(target_info['data']['allTarget']):
    ...
    time.sleep(2)
    # 指标表头+单位对比
    header_title = re.findall(r'''<div class="header-title">\s+(.*?)\s+</div>''', driver.page_source)[0]
    if header_title != target_dic['header_title']:
        print(target_dic['indicatorname'] + '标题表头显示有误')

    if target_dic['title'] not in driver.page_source:
        print(target_dic['indicatorname'] + '表格表头显示有误')

    # 指标年份对比
    if target_dic['dataYear'] == '\\':
        if 'class="filter-school" style="display: none' not in driver.page_source:
            print(target_dic['indicatorname'] + '默认年份显示有误')
    else:
        if target_dic['yearrange'] not in driver.page_source:
            print(target_dic['indicatorname'] + '默认年份显示有误')
